...
--------------------------------------------------------------------------------
  ...
  A2_Squirrel_Analysis_v4.ipynb   Main notebook (all analysis code)
...
Required data files (place in the same folder as the notebook):
  ...
  stories.csv                     809 qualitative field notes (loaded but not
                                  used in analysis — included for completeness)
...
4. Run all cells top to bottom in order:
   ...
   or manually run each cell from top to bottom.

   Do NOT run cells out of order — later cells depend on variables
   defined in earlier cells.

Expected runtime: approximately 5-20 seconds (full run).
...
--------------------------------------------------------------------------------
  ...
  Section 1   Load data (squirrel.csv, hectare.csv, stories.csv)
  ...
  Section 6   Clustering
              ...
              6.7  Approach rate per cluster (RQ connection)
...
All figures are saved as .png files in the same directory as the notebook:
...
All numerical results printed in cell outputs correspond directly to the values
reported in the submitted report (A2_Report_Final.pdf).

--------------------------------------------------------------------------------
NOTES
--------------------------------------------------------------------------------

- The data file path is configurable via the DATA_DIR variable at the top of
  Section 1 (default: '.', meaning same folder as the notebook).

- All random operations use random_state=42 for reproducibility.

- Clustering is performed on the full dataset (df, n=3023), not the balanced
  subset, to capture the full range of behavioural variation.

- The KNN Pipeline ensures scaling is fitted only on training data within each
  CV fold, preventing data leakage.

- stories.csv is loaded but not used in the analysis. It is included in the
  load step for completeness and potential future use.
